...
// Оновлений маршрут для оновлення монет
app.post('/api/updateUserCoins', async (req, res) => {
  ...
  try {
    ...
    // Перевірка рівня після оновлення монет
    const userData = await getUserData(userId);
    const currentLevel = userData.level;

    res.json({
      success: true,
      newCoins: result.newCoins.toString(),
      newTotalCoins: result.newTotalCoins.toString(),
      currentLevel
    });
  } catch (error) {
